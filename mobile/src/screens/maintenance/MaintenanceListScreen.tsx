...
const MaintenanceListScreen = ({ navigation }: any) => {
  ...
  useEffect(() => {
    ...
    setFiltered(records.filter((r: any) =>
      r.vehiclePlate?.toLowerCase().includes(q) || r.description?.toLowerCase().includes(q) || r.type?.toLowerCase().includes(q)
    ));
  }, [search, records]);

  const totalCost = records.reduce((sum, r) => sum + (r.cost || r.amount || 0), 0);
  ...
  return (
    <View style={styles.container}>
      <View style={styles.summaryCard}>
        <Icon name="wrench-outline" size={24} color={colors.white} />
        <View style={styles.summaryInfo}>
          <Text style={styles.summaryLabel}>Total Maintenance</Text>
          <Text style={styles.summaryAmount}>AED {totalCost.toLocaleString()}</Text>
        </View>
      ...
      <FlatList
        ...
        ListEmptyComponent={<EmptyState icon="wrench" title="No maintenance records" />}
        renderItem={({ item }) => (
          <Card>
            <View style={styles.row}>
              <View style={styles.iconBox}>
                <Icon name="wrench-outline" size={22} color={colors.error} />
              </View>
              <View style={styles.info}>
                <Text style={styles.title}>{item.description || item.type || 'Maintenance'}</Text>
                <Text style={styles.sub}>{item.vehiclePlate || ''} {item.date ? `· ${item.date}` : ''}</Text>
              </View>
              <View style={styles.right}>
                <Text style={styles.amount}>AED {(item.cost || item.amount || 0).toLocaleString()}</Text>
...
const styles = StyleSheet.create({
  ...
  iconBox: { width: 44, height: 44, borderRadius: 12, backgroundColor: colors.errorLight, justifyContent: 'center', alignItems: 'center' },
  ...
  amount: { fontSize: fontSize.md, fontFamily: fonts.bold, color: colors.error },
});
